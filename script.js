...
let dartThrows = 0;
let totalScore = 0;
let hits = []; // Flytta hits hit för att behålla historiken
...
if (!dartBoardBtn) {
  console.error('Knappen #dartBoardBtn hittades inte i DOM!');
} else {
  console.log('Knapp #dartBoardBtn hittad');
  dartBoardBtn.addEventListener('click', () => {
    console.log('Knapp tryckt!');
    const dartModal = document.getElementById('dartModal');
    if (!dartModal) {
      console.error('Modal #dartModal hittades inte!');
    } else {
      console.log('Modal ska visas');
      dartModal.style.display = 'flex';
      dartModal.classList.remove('hidden');
      dartThrows = 0;
      totalScore = 0;
      hits = []; // Nollställ hits när modalen öppnas
      document.getElementById('scoreDisplay').textContent = 'Poäng: 0';
...
document.getElementById('closeModal').addEventListener('click', () => {
  ...
  if (dartModal) {
    ...
    dartThrows = 0;
    totalScore = 0;
    hits = []; // Nollställ hits när modalen stängs
  }
});

// Följ mus/touch utan offset
dartBoard.addEventListener('mousemove', (event) => {
  const rect = dartBoard.getBoundingClientRect();
  const x = event.clientX - rect.left;
  const y = event.clientY - rect.top;
  updateCursorPosition(x, y, rect);
});
...
function updateCursorPosition(x, y, rect) {
...
// Lägg till SVG-cirklar för att visa zonerna


// Registrera poäng vid dubbelklick med sektorkontroll
dartBoard.addEventListener('dblclick', (event) => {
  console.log('Dubbelklick detekterat på dartBoard');
  if (dartThrows < 3) {
    event.preventDefault();
    const rect = dartBoard.getBoundingClientRect();
    const x = event.clientX - rect.left;
    ...
    // Vinkel: 0° = kl 12, ökar medsols, vridning +9° för riktig placering
    let rawAngle = Math.atan2(dy, dx) * (180 / Math.PI);
    let angle = (rawAngle + 360 + 90 + 9) % 360;

    // Darttavlans ordning (medsols från 20)
    const sectorMap = [20, 1, 18, 4, 13, 6, 10, 15, 2, 17,
                       3, 19, 7, 16, 8, 11, 14, 9, 12, 5];
    const sectorIndex = Math.floor(angle / 18);
    const sector = sectorMap[sectorIndex] || 20;

    // Normaliserat avstånd (0 = centrum, 1 = ytterkant)
    const rNorm = distance / maxRadius;
    ...
    if (rNorm <= 0.05) {
      ...
      score = sector;
      type = `S${sector}`; // Single (inner)
    } else if (rNorm <= 0.46) {
      score = sector * 3;
      type = `T${sector}`; // Triple
    } else if (rNorm <= 0.70) {
      score = sector;
      type = `S${sector}`; // Single (outer)
    } else if (rNorm <= 0.77) {
      score = sector * 2;
      type = `D${sector}`; // Double
    } else {
      score = 0;
      type = "MISS";
    }

    console.log(`Sector: ${sector}, Type: ${type}, Score: ${score}, rNorm: ${rNorm}, Distance: ${distance}, MaxRadius: ${maxRadius}`);

    // Räkna alla kast, även missar
dartThrows++;
totalScore += score; // score är 0 för miss, så totalScore påverkas inte
hits.push(type);

// Skapa en sträng med alla träffar (begränsa till de 3 senaste)
let displayHits = hits.slice(-3).join(",");

// Uppdatera displayen
document.getElementById('scoreDisplay').textContent =
    `Poäng: ${totalScore} - ${displayHits} - (Kast ${dartThrows}/3)`;
  }
...
// Funktion för att hämta sektorvärde (nu överflödig, men behålles för debug)
function getSectorScore(x, y, element) {
  let sectorNumber = 0;
  const textElements = dartBoard.getElementsByClassName('cls-2');
  const rect = dartBoard.getBoundingClientRect();
  const viewBox = dartBoard.viewBox.baseVal;
  ...
  for (let text of textElements) {
    ...
    console.log('Kontrollerar text:', text.textContent,
                'scaledBBox:', scaledBBox,
                'Click coords:', { x, y });
    if (x >= scaledBBox.x && x <= scaledBBox.x + scaledBBox.width &&
        y >= scaledBBox.y && y <= scaledBBox.y + scaledBBox.height) {
      sectorNumber = parseInt(text.textContent.replace(/\s/g, '')) || 0;
      console.log('Träff på sektor:', sectorNumber);
      break;
    }
  }
  return sectorNumber;
